...
func _ready():
	...
	originScale = scale
	prevPosition = global_transform
	currentPositon = global_transform
...
func pickmeUp():
	if is_picked_up:
		return
	EventBus.emitCustomSignal("object_held", [0, self])
	is_picked_up = true
	self.reparent(player, true)
	#position = itemPos.global_transform.origin

func dropMe():
	if is_picked_up:
		EventBus.emitCustomSignal("dropped_object",[0])
...
func update_position(delta):
	if is_picked_up:
		...
		#ar angleBetweenForceAndVelocity = min(90,force.angle_to(parent.linear_velocity))*2
