...
func update_rotation(delta):
	...
	if horizontal_input.length() > 0.1 and input_dir.y != 0:
		adjust_rotation(input_dir, velocity_difference, delta)
	else:
...
func adjust_rotation(input_dir, velocity_difference, delta):
	if velocity_difference.z < 0:
		_rotate = lerp(_rotate, deg_to_rad(0.5 * -input_dir.x * persistent_state.velocity.length()), delta * 5)
	else:
		_rotate = lerp(_rotate, deg_to_rad(0.5 * input_dir.x * persistent_state.velocity.length()), delta * 5)
