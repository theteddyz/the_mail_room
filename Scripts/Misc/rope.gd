...
@onready var phone_body = find_child("PhoneBody")
@onready var phone_reciever = find_child("PhoneReciever")
...
func _ready()->void:
	...
	pass
	#Connect the phone body
	#ROPE_BODIES.append(phone_body)
	#add_child(phone_body)
	#Adding a PinJoint between RigidBodies.
	#phone_body.transform.origin = ROPE_BODIES[ROPE_BODIES.size()-1].transform.origin
...
func _add_pin_joint(node_A, node_B)->void:
	...
	pin.set("angular_limit_x/enabled",false)
	
	#pin.set("linear_limit_x/enabled", true)
	#pin.set("linear_limit_y/enabled", true)
	#pin.set("linear_limit_z/enabled", true)
	
	#pin.set("linear_spring_x/enabled", true)
	#pin.set("linear_spring_y/enabled", true)
	#pin.set("linear_spring_z/enabled", true)
	
	#pin.set("linear_spring_x/stiffness", 0.01)
	#pin.set("linear_spring_y/stiffness", 0.01)
	#pin.set("linear_spring_z/stiffness", 0.01)
	
	#pin.set("angular_limit_y/enabled",false)
	#pin.set("angular_limit_z/enabled",false)
	...
	if node_A == null:
		#pin.transform.origin = Vector3(phone_reciever.get_position().x, phone_reciever.get_position().y-20,phone_reciever.get_position().z)
		pin.set_node_a(phone_reciever.get_path())
	else:
