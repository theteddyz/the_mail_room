...
func is_occluded(object) -> bool:
	...
	raycast_params.to = from_position
	raycast_params.from = to_position

	# Set the collision mask to only check against layer 8
	raycast_params.set_collision_mask(1)  # Layer 16 corresponds to bit 15
	...
	if result.has("collider") and result["collider"] != object:
		#if(result.collider.name == "floor_staticbody"):
		#	return false
		#else:
		#	return true	
